...
ChatBot.SERVER_PATH = "http://127.0.0.1:7050";
...
//Holds the speeach synthesis configuration like language, pich and rate
...
ChatBot.initSpeechConfig = function(){
	if (ChatBot.speechEnabled ){
		...
		ChatBot.speechConfig.pitch = 1.6;
		ChatBot.speechConfig.onend = function(event) { $("#speak-indicator").addClass("hidden"); }
	}
};
...
ChatBot.write = function(message,sender, emoji){
	//Ony boto's messages should be heard
	if (sender == "boto" && ChatBot.speechEnabled){
		ChatBot.speak(message);
	}
	var chatScreen = $(".chat-screen");
	var sender = $("<span />").addClass("sender").addClass(sender).text(sender + ":");
